...
def create_corpus_routes(api: falcon.App, context: Context):
    ...
    chapters_display = ChaptersDisplayResource(corpus, context.cache)
    chapters_line = LineResource(corpus)
    chapters_by_lemma = ChaptersByLemmaResource(corpus)
    alignment = AlignmentResource(corpus, context.cache)
    manuscript_lemmatization = LemmatizationResource(corpus, context.cache)
    manuscript = ManuscriptsResource(corpus, context.cache)
    lines = LinesResource(corpus, context.cache)
    lines_import = LinesImportResource(corpus, context.cache)
