...
def parse_annotations(annotation_data: AnnotationData) -> str:
    ...
    if annotation_data.sign_name != "":
        ...
        if annotation_data.value.isdigit():
            ...
            # sometimes reading is there but no sign (these are matched manually)
            # mostly just lower to uppercase
            if annotation_data.value in MANUEL_FIX:
                return MANUEL_FIX[annotation_data.value]
            else:
                print(annotation_data.value)
                return "STRUCT"
...
if __name__ == "__main__":
    ...
    # For Sign Detection aka Bounding Boxes use first TO_FILTER.
    # For retrieving only signs for Sign Classification use second TO_FILTER
    # which includes Unclear Sign
    # TO_FILTER = [AnnotationValueType.RULING_DOLLAR_LINE, AnnotationValueType.ColumnAtLine]
    TO_FILTER = [
