...
def create_annotations(
    ...
    from top left.
    Original format from react-annotation tool is bottom left vertice and height and
    width values between 0 - 100 relative to image size.
    We compare number of annotations to transliteration so we can exclude annotaions
...
def save_image(
    fragment_number, cdli_number, output_folder, photo_repository: FileRepository
):
    """
    Some images are in our database, some aren't so we have to retrieve them from CDLI.
    Images from CDLI can have different shape than ours so if we can we try to retrieve
    them from our db.
    """
