...
class AnnotationImageExtractor:
    ...
    def cropped_images_from_sign(self, sign: str) -> Sequence[CroppedAnnotation]:
        ...
        for single_annotation in annotations:
            ...
            for annotation in single_annotation.annotations:
                script, label = self._get_info(annotation,
                                                   fragment_number)
                cropped_image = self._crop_from_annotation(annotation, fragment_number)
                cropped_image_result = CroppedAnnotation(cropped_image,fragment_number, script, label)
    ...
    def _crop_from_annotation(self, annotation: Annotation, fragment_number: MuseumNumber) -> Base64:
        ...
        area = (bounding_boxes.top_left_x, bounding_boxes.top_left_y, bounding_boxes.top_left_x + bounding_boxes.width,
