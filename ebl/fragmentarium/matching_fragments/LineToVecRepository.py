...
class MongoLineToVecRepository(LineToVecRepository):
    ...
    def insert_all(self, entries: dict):
        self._collection.update_one({"_id": "lineToVec"}, {"$set": {"lineToVec": {k: LineToVecSchema().dump(v) for k, v in entries.items()}}})

    def get_line_to_vec_with_rulings(self):
        doc = self._collection.find_one_by_id("lineToVec")
        return {k: LineToVecSchema().load(v) for k, v in doc["lineToVec"].items() if v["complexity"] > 0}
