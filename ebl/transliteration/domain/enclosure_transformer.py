...
class EnclosureTransformer(Transformer):
    def ebl_atf__text_line__open_accidental_omission(self, _):
        return AccidentalOmission.open()

    def ebl_atf__text_line__close_accidental_omission(self, _):
        return AccidentalOmission.close()

    def ebl_atf__text_line__open_intentional_omission(self, _):
        return IntentionalOmission.open()

    def ebl_atf__text_line__close_intentional_omission(self, _):
        return IntentionalOmission.close()

    def ebl_atf__text_line__open_removal(self, _):
        return Removal.open()

    def ebl_atf__text_line__close_removal(self, _):
        return Removal.close()

    def ebl_atf__text_line__close_broken_away(self, _):
        return BrokenAway.close()

    def ebl_atf__text_line__open_broken_away(self, _):
        return BrokenAway.open()

    def ebl_atf__text_line__close_perhaps_broken_away(self, _):
        return PerhapsBrokenAway.close()

    def ebl_atf__text_line__open_perhaps_broken_away(self, _):
        return PerhapsBrokenAway.open()

    @v_args(inline=True)
    def ebl_atf__text_line__open_document_oriented_gloss(self, _):
        return DocumentOrientedGloss.open()

    @v_args(inline=True)
    def ebl_atf__text_line__close_document_oriented_gloss(self, _):
        return DocumentOrientedGloss.close()

    @v_args(inline=True)
    def ebl_atf__text_line__erasure(self, erased, over_erased):
...
class GlossTransformer(Transformer):
    @v_args(inline=True)
    def ebl_atf__text_line__determinative(self, tree):
        tokens = tokens_to_value_tokens(tree.children)
        return Determinative.of(tokens)

    @v_args(inline=True)
    def ebl_atf__text_line__phonetic_gloss(self, tree):
        tokens = tokens_to_value_tokens(tree.children)
        return PhoneticGloss.of(tokens)

    @v_args(inline=True)
    def ebl_atf__text_line__linguistic_gloss(self, tree):
        tokens = tokens_to_value_tokens(tree.children)
