...
class AtLineTransformer(LabelTransformer):
    def ebl_atf_text_line__free_text(self, content):
        return "".join(content)

    @v_args(inline=True)
    def ebl_atf_text_line__INT(self, number):
        return int(number)
    ...
    @v_args(inline=True)
    def ebl_atf_text_line__generic_object(self, text: str):
        return ObjectData(atf.Object.OBJECT, text)

    @v_args(inline=True)
    def ebl_atf_text_line__fragment(self, text: str):
    ...
    @v_args(inline=True)
    def ebl_atf_text_line__surface_with_status(self, surface: SurfaceLabel, statuses):
        return SurfaceAtLine(SurfaceLabel(statuses, surface.surface, surface.text))

    @v_args(inline=True)
    def ebl_atf_text_line__object_with_status(self, object_: ObjectData, statuses):
        return ObjectAtLine(ObjectLabel(statuses, object_.object, object_.text))

    @v_args(inline=True)
    def ebl_atf_text_line__divisions(self, text, digit):
        return DivisionAtLine(text, digit)

    @v_args(inline=True)
    def ebl_atf_text_line__composite_start(self, text, number):
        return CompositeAtLine(atf.Composite.DIV, text, number)

    @v_args(inline=True)
    def ebl_atf_text_line__composite_end(self, text):
        return CompositeAtLine(atf.Composite.END, text)

    @v_args(inline=True)
    def ebl_atf_text_line__composite_composite(self):
        return CompositeAtLine(atf.Composite.COMPOSITE, "")

    @v_args(inline=True)
    def ebl_atf_text_line__composite_milestone(self, text, number):
