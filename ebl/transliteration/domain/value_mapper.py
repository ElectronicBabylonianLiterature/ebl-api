...
def map_reading(match):
    sub_index = sub_index_to_int(match.group(2)) if match.group(2) else 1
    return ValueFactory.create_reading(match.group(1), sub_index)


def map_variant(match):
    return ValueFactory.create_variant(
        tuple(
            parse_reading(part, True)
            for part in match.group(0).split(VARIANT_SEPARATOR)
        )
    )
