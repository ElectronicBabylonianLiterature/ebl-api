...
class ParallelLineTransformer(LabelTransformer):
    @v_args(inline=True)
    def text_line__parallel_fragment(
        self,
        _prefix,
        cf,
        museum_number,
    ...
    @v_args(inline=True)
    def text_line__chapter_name(self, stage_abbreviation, version, name) -> ChapterName:
        return ChapterName(
    ...
    def text_line__parallel_composition(self, children) -> ParallelComposition:
