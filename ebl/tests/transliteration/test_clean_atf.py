...
def test_strip_line_numbers():
    clean_atf = CleanAtf(Atf("1. mu\n2'. me\na+1. e\n1.2. a\n11'-12'. kur. ra"))
    assert clean_atf.cleaned == [["mu"], ["me"], ["e"], ["a"], ["kur", "ra"]]
...
def test_indent():
    ...
    assert clean_atf.cleaned == [["ša₂"]]


def test_strip_flags():
    clean_atf = CleanAtf(Atf("1.  ba! ba? ba# ba*"))
    assert clean_atf.cleaned == [["ba", "ba", "ba", "ba"]]


def test_strip_shifts():
    clean_atf = CleanAtf(Atf("1. %es qa\n2. ba %g ba"))
    assert clean_atf.cleaned == [["qa"], ["ba", "ba"]]


def test_strip_commentary_protocols():
    clean_atf = CleanAtf(Atf("1. !qt qa !zz\n2. ba !cm ba !bs"))
    assert clean_atf.cleaned == [["qa"], ["ba", "ba"]]


def test_strip_omissions():
    clean_atf = CleanAtf(Atf("1.  <NU> KU₃\n2. <(ba)> an\n5. <<a>> ba"))
    assert clean_atf.cleaned == [["ku₃"], ["an"], ["ba"]]


def test_min():
    clean_atf = CleanAtf(Atf("3. MIN<(an)> ši"))
    assert clean_atf.cleaned == [["min", "ši"]]


def test_numbers():
    clean_atf = CleanAtf(Atf("1. 1(AŠ)\n2. 1 2 10 20 30\n3. 256"))
    assert clean_atf.cleaned == [
        ["1(AŠ)"],
        ["1", "2", "10", "20", "30"],
        ["256"],
...
def test_graphemes():
    ...
    clean_atf = CleanAtf(
        Atf(
            "\n".join(
                [f"{index}. {grapheme}" for index, grapheme in enumerate(graphemes)]
            )
        )
    )
