...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("foo(|BI.IS|)", SplittableGrapheme.of([SignName("BI"), SignName("IS")]),),
        (
            "šu/gid₂",
            Variant(
                (Reading("šu", 1, INVALID_READING), Reading("gid", 2, INVALID_READING),)
            ),
        ),
        (
            "šu/gid₂/nu",
        ...
        (
            "šu/|BI.IS|",
            Variant(
                (Reading("šu", 1, INVALID_READING), Grapheme(SignName("|BI.IS|")),)
            ),
        ),
        (
            "šu/|BI×IS|",
        ...
        (
            "|BI×IS|/šu",
            Variant(
                (Grapheme(SignName("|BI×IS|")), Reading("šu", 1, INVALID_READING),)
            ),
        ),
        (
            "šu/|BI×IS|/nu",
        ...
        (
            "foo(TUKUL)/šu",
            Variant((Grapheme(SignName("TUKUL")), Reading("šu", 1, INVALID_READING),)),
        ),
        (
            "foo(|BI×IS|)/šu",
            Variant(
        ...
        (
            "šu/1(AŠ)",
            Variant((Reading("šu", 1, INVALID_READING), Grapheme(SignName("AŠ")))),
        ),
        (
            "256/nu",
            Variant((Reading("256", 1, "256"), Reading("nu", 1, INVALID_READING))),
        ),
        ...
        (
            "unknown/x",
            Variant((Reading("unknown", 1, INVALID_READING), NotReading("X"))),
        ),
        ("", NotReading("")),
    ],
)
...
def test_nested_variants_are_invalid():
    with pytest.raises(ValueError):
        Variant(
            (
                Variant((Reading("kur", 1, INVALID_READING), NotReading("X"),)),
                NotReading("X"),
            )
        )
...
def test_variant_cannot_contain_splittable_grapheme():
    with pytest.raises(ValueError):
        Variant(
            (
                SplittableGrapheme(
                    (Grapheme(SignName("BI")), Grapheme(SignName("BI")))
                ),
                NotReading("X"),
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        NotReading("X"),
        Variant((Reading("kur", 1, INVALID_READING), NotReading("X"))),
        SplittableGrapheme((Grapheme(SignName("BI")), Grapheme(SignName("BI")))),
    ],
)
