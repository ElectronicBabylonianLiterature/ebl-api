...
def test_find_by_abbreviation(provenance_repository):
    clear_provenances(provenance_repository)
    record = ProvenanceRecord(
        id="TEST_NINEVEH",
        long_name="Test Nineveh",
        abbreviation="Nin",
    )
    provenance_repository.create(record)

    service = ProvenanceService(provenance_repository)
    result = service.find_by_abbreviation("Nin")

    assert result is not None
    assert result.id == "TEST_NINEVEH"


def test_find_by_id(provenance_repository):
    clear_provenances(provenance_repository)
    record = ProvenanceRecord(
        id="TEST_URUK",
        long_name="Test Uruk",
    ...
    service = ProvenanceService(provenance_repository)
    result = service.find_by_id("TEST_URUK")

    assert result is not None
    assert result.long_name == "Test Uruk"


def test_find_children(provenance_repository):
    clear_provenances(provenance_repository)
    provenance_repository.create(
        ProvenanceRecord(
            id="TEST_PARENT",
            long_name="Test Parent",
            abbreviation="Tpa",
        )
    )
    provenance_repository.create(
    ...
    service = ProvenanceService(provenance_repository)
    result = service.find_children("Test Parent")

    assert {record.id for record in result} == {"TEST_CHILD_1", "TEST_CHILD_2"}
